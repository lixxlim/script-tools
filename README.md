...
- `git-clean-branches`: 일정 기간(기본 30일) 동안 커밋이 없는 브랜치를 찾아 선택한 후, 로컬과 리모트에서 동시에 삭제합니다. (`git`, `fzf` 필요)
...
`crontab/` 디렉토리에는 주기적으로 실행할 스크립트들이 포함되어 있으며, 실행 주기에 따라 폴더별로 구분되어 있습니다.

- **01_daily**: 매일 실행할 스크립트
- **02_weekly**: 매주 실행할 스크립트
- **03_monthly**: 매월 실행할 스크립트
- **04_yearly**: 매년 실행할 스크립트

### 현재기준 스케줄러 목록

- **check_oreilly** (01_daily): O'Reilly Japan eBook 페이지의 변경 사항을 감지하여 GitHub Actions 워크플로우를 실행합니다. (`gh` CLI, `curl` 필요)
